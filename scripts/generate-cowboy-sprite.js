/**
 * Cowboy Sprite Generator
 *
 * This script generates a cowboy character sprite sheet with animations
 * for a space western game. The cowboy has classic western styling.
 */
...
console.log('Generating cowboy sprite sheet...');
...
// Color palette
const colors = {
	skin: '#d2a679', // Tan skin tone
	hatBrown: '#5e3515', // Dark brown hat
	hatBand: '#222222', // Black hat band
	shirt: '#336699', // Blue denim shirt
	pants: '#993300', // Brown pants
	vest: '#994422', // Brown leather vest
...
// Helper function to draw the cowboy character
function drawCowboy(frameX, frameY, poseType, frameNum) {
	...
	// Left leg
	ctx.beginPath();
	ctx.roundRect(x - 8 - legOffset, y - 18, 6, 12, 2);
	ctx.fill();

	// Right leg
	ctx.beginPath();
	ctx.roundRect(x + 2 + legOffset, y - 18, 6, 12, 2);
	ctx.fill();
...
for (let i = 0; i < 4; i++) {
	drawCowboy(i, 0, 'idle', i);
}

// Draw running animation frames (second row)
for (let i = 0; i < 8; i++) {
	drawCowboy(i % 4, 1 + Math.floor(i / 4), 'run', i);
}

// Draw jumping animation frames (bottom row)
for (let i = 0; i < 4; i++) {
	drawCowboy(i, 3, 'jump', i);
}

// Save the image
const buffer = canvas.toBuffer('image/png');
fs.writeFileSync(path.join(imagesDir, 'cowboy.png'), buffer);

console.log('Cowboy sprite sheet generated successfully!');
console.log('Saved to: assets/images/cowboy.png');
